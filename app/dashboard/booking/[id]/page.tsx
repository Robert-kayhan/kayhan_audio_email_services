  ...
  useEffect(() => {
    if (data?.booking) {
      setFormData({
        ...
        mobileDetails: data.booking.MobileInstallationDetail || {
          address: "",
          city: "",
          state: "",
          postcode: "",
      ...
      >
        ...
        {formData.booking.installationType === "Mobile" && (
          <section>
            ...
            <div className="grid grid-cols-2 gap-4">
              {Object.keys(formData.mobileDetails).map((key) => (
                <input
                  key={key}
                  type="text"
                  value={formData.mobileDetails[key] || ""}
                  onChange={(e) =>
                    handleChange("mobileDetails", key, e.target.value)
                  }
                  placeholder={key}
                  className="border rounded px-3 py-2 w-full"
                />
        ...
        <section>
          ...
          <button
            ...
            onClick={() =>
              setFormData((prev: any) => ({
                ...prev,
                items: [...prev.items, { itemType: "", otherItemText: "" }],
              }))
            }
