...
import { useUpdateProductMutation } from "@/store/api/Inventory/productApi";
...
import { useGetProductByIdQuery } from "@/store/api/Inventory/productApi";
...
const UpdateProductPage = () => {
   ...
   useEffect(() => {
    if (productData?.data) {
      ...
      reset({
        ...
        factory_price: p.factory_price || 0,
        ...
        width: p.width || 0,
        images: p.images || [],
        car_model_id: p.car_model_id || undefined,
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const result: any = await updateProduct({ id,data}).unwrap();
