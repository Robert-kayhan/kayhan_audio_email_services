...
import CreateModel from "@/components/Inventory/channel/CreateChannelModel";
...
const ChannelPage = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetChannelsQuery({
    page: currentPage,
    limit,
    search,
  });
  ...
  const handleDelete = async (row: Channel) => {
    try {
      await deleteChannel(row.id).unwrap();
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center px-3 py-2 gap-4 flex-wrap">
        <h2 className="text-2xl font-serif font-bold">Channels</h2>
